...
To predict this I include 3 different classification algorithms in the analysis: logistic regression, k-nearest neighbors (kNN) and Support Vector Machines. All 3 are well suited for when the outcome variable is binary, as is the case in this analysis (churn/no churn). 
...
### Logistic regression
Logistic regression is a variant of the general linear model. It's an extension of linear regression that outputs conditional probabilities of the outcome variable given the predictor variable(s) (Irizarry 2019:563). In classificatione these probabilities are used to predict class membership. Unlike linear regression that uses least squares, logistic regression uses the maximum likelihood estimate. But like linear regression, the decision rule is line (Irizarry 2019:567).
...
based on bin smoothing (Irizarry 2019:532).

"First we define the distance between all observations based on the features. Then, for any point (x1,x2) for which we want an estimate of p(x1,x2), we look for the k nearest points to (x1,x2) and then take an average of the 0s and 1s associated with these points. We refer to the set of points used to compute the average as the neighborhood. Due to the connection we described earlier between conditional expectations and conditional probabilities, this gives us a pˆ(x1,x2), just like the bin smoother gave us an estimate of a trend. As with bin smoothers, we can control the flexibility of our estimate, in this case through the k parameter: larger ks result in smoother estimates, while smaller ks result in more flexible and more wiggly estimates" (Irizarry 2019:536).
...
- **Overall accuracy**: the fraction of cases that were correctly predicted in the test set (Kabacoff 2021:465). Calculated as the number of correct predictions divided by the total number of data points.
...
- **Specificity (or true negative rate)**: The proportion of negatives that are predicted as negatives (Irizarry 2019:508). In this analysis it's the percentage of people who are still using the service that are correctly identified as such. Calculated as the number of true negatives divided by the number of true negatives + the number of false positives.
...
"The ROC curve plots sensitivity (TPR) versus 1 - specificity or the false positive rate (FPR)" (Irizarry 2019:511).
