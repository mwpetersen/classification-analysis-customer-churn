...
To predict this I include 3 different classification algorithms in the analysis: logistic regression, K-Nearest Neighbors and Support Vector Machines. All 3 are well suited for when the outcome variable is binary, as is the case in this analysis (churn/no churn).  
...
# Method
In this section I describe how I will go about predicting customer churn from the telecom data set. Specifically, I describe the data, the process of supervised machine learning and the algorithms used in this analysis.
...
::: {}
- `r colnames(py$df_churn)[1]`
- `r colnames(py$df_churn)[2]`
- `r colnames(py$df_churn)[3]`
- `r colnames(py$df_churn)[4]`
:::

::: {}
- `r colnames(py$df_churn)[5]`
- `r colnames(py$df_churn)[6]`
- `r colnames(py$df_churn)[7]`
- `r colnames(py$df_churn)[8]`
:::

::: {}
- `r colnames(py$df_churn)[9]`
- `r colnames(py$df_churn)[10]`
- `r colnames(py$df_churn)[11]`
:::
...
In supervised machine learning the starting point is a data set that include both the predictor variables and the outcome variable. A key idea is to then divide the data set into a training set and a test set. The training set is used to fit the statistical model to the data. Afterwards the test set is used to evaluate how good the model is at predicting the outcome variable of interest (Kabacoff 2021:449). This is done by using the predictor variables in the test set and the trained statistical model to predict the class of each observation in the test set. These predictions are then compared to the actual values of the outcome variable in the test set. This evaluation is used to assess how good the model will perform in future situations where we only have access to a set of predictor variables and not the actual outcome variable.

## Classification algorithms used in this analysis
Choosing an algoritm

K-Nearest Neighbors (KNN)
The basic idea of KNN is to predict the class of a data point by looking at the 'k' closest data points. The predicted class is the class with the highest frequency among the k closest data points.
...
- **Accuracy**: the fraction of correct predictions (the number of correct predictions divided by the total number of data points). Or in other words how often the classifier makes a correct prediction (Kabacoff 2021:465).

- **Precision (or positive predictive value)**: “Probability that an observation with a positive classification is correctly identified as positive” (Kabacoff 2021:465). So if a person is classified as having quit the service how likely is it that this classification will be correct? In other words – in the case of customer churn – high precision means that the algorithm rarely predicts churn when the customer has in fact stayed. Calculated as the number of true positives divided by the number of true positives + the number of false positives.

- **Sensitivity (or recall or true positive rate)**: “Probability of getting a positive classification when the true outcome is positive” (Kabacoff 2021:465). What percentage of people who have stopped using the service are correctly identified? In other words – In the case of customer churn – high recall means that the algorithm has predicted most churn correctly. Calculated as the number of true positives divided by the number of true positives + the number of false negatives.

- **Specificity (or true negative rate)**: “Probability of getting a negative classification when the true outcome is negative” (Kabacoff 2021:465). What percentage of people who are still using the service are correctly identified? Calculated as the number of true negatives divided by the number of true negatives + the number of false positives.
