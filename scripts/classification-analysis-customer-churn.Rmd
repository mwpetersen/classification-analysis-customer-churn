...
SVM's are based on the concept of so-called separating hyperplanes (James et al. 2021:369). The algorithm uses these hyperplanes to determine decision rules. The role of a hyperplane is to maximize the margin between the closest points of two classes (Kabacoff 2021:460). Data points closest to the hyperplane define the boundary of the margins of the hyperplane. These are called support vectors. The most simple hyperplane is a line (in a 2 dimensional space), but it can also take the form of a N-dimensional hyperplane, depending on the data (Kabacoff 2021:460). When there is a non-linear relationship between the data of interest, SVM uses so-called kernels to "tranform the data into higher dimensions" as Kabacoff (2021:460) formulates it.
